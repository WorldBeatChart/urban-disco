...
class _ChartScreenState extends State<ChartScreen> {
  ...
  void _onScroll() {
    ...
    if (t != _showScrollTop || b != _showScrollBottom) setState(() { _showScrollTop = t; _showScrollBottom = b; });
  }
  ...
  Future<void> _init() async {
    ...
    if (_genres.isEmpty) {
      _genres = [
        DeezerGenre(id: 0, name: 'All'), DeezerGenre(id: 132, name: 'Pop'),
        DeezerGenre(id: 116, name: 'Rap/Hip Hop'), DeezerGenre(id: 152, name: 'Rock'),
        DeezerGenre(id: 113, name: 'Dance'), DeezerGenre(id: 165, name: 'R&B'),
        DeezerGenre(id: 85, name: 'Alternative'), DeezerGenre(id: 106, name: 'Electro'),
        DeezerGenre(id: 466, name: 'Folk'), DeezerGenre(id: 464, name: 'Metal'),
        DeezerGenre(id: 129, name: 'Jazz'), DeezerGenre(id: 98, name: 'Classical'),
        DeezerGenre(id: 84, name: 'Country'), DeezerGenre(id: 144, name: 'Reggae'),
        DeezerGenre(id: 169, name: 'Soul & Funk'), DeezerGenre(id: 2, name: 'African'),
        DeezerGenre(id: 16, name: 'Asian'), DeezerGenre(id: 75, name: 'Brazilian'),
        DeezerGenre(id: 81, name: 'Indian'), DeezerGenre(id: 197, name: 'Latin'),
        ...
        CustomScrollView(controller: _scrollCtrl, physics: const BouncingScrollPhysics(), slivers: [
          SliverToBoxAdapter(child: Container(
            decoration: BoxDecoration(
              ...
              image: const DecorationImage(image: NetworkImage('https://images.pexels.com/photos/196652/pexels-photo-196652.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1'),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                child: TextField(controller: _artistCtrl, style: GoogleFonts.inter(fontSize: 14, color: Colors.white),
                  decoration: InputDecoration(
                    ...
                    suffixIcon: _artistCtrl.text.isNotEmpty
                      ? IconButton(icon: const Icon(Icons.close_rounded, color: Colors.white38, size: 18),
                          onPressed: () { _artistCtrl.clear(); _artistQuery = ''; _load(); }) : null),
          ...
          if (_loading) const SliverFillRemaining(child: Center(child: CircularProgressIndicator(color: Color(0xFF818cf8))))
            ...
            Text(_error!, style: GoogleFonts.inter(color: Colors.white70)), const SizedBox(height: 12),
            ElevatedButton(onPressed: _load, child: const Text('Retry'))])))
          else if (_albums.isEmpty) SliverFillRemaining(child: Center(child: Text('No albums found', style: GoogleFonts.inter(color: Colors.white38, fontSize: 15))))
          else SliverPadding(padding: const EdgeInsets.all(16), sliver: SliverGrid(
            gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 220, mainAxisSpacing: 16, crossAxisSpacing: 16, childAspectRatio: 0.7),
            delegate: SliverChildBuilderDelegate((c, i) => _AC(a: _albums[i]).animate().fadeIn(delay: Duration(milliseconds: i * 40), duration: 300.ms).slideY(begin: 0.05, end: 0),
              childCount: _albums.length))),
          const SliverToBoxAdapter(child: SizedBox(height: 32)),
        ]),
        if (_showScrollTop) Positioned(right: 16, bottom: 60, child: _SB(icon: Icons.keyboard_arrow_up_rounded,
          onTap: () => _scrollCtrl.animateTo(0, duration: const Duration(milliseconds: 500), curve: Curves.easeOut))),
        if (_showScrollBottom) Positioned(right: 16, bottom: 16, child: _SB(icon: Icons.keyboard_arrow_down_rounded,
          onTap: () => _scrollCtrl.animateTo(_scrollCtrl.position.maxScrollExtent, duration: const Duration(milliseconds: 500), curve: Curves.easeOut))),
      ])));
  }
}
    ...
    return MouseRegion(cursor: SystemMouseCursors.click,
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(duration: const Duration(milliseconds: 200),
          ...
          decoration: BoxDecoration(color: _h ? const Color(0xFF1e293b) : const Color(0xFF162032), borderRadius: BorderRadius.circular(16),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ClipRRect(borderRadius: const BorderRadius.only(topLeft: Radius.circular(16), topRight: Radius.circular(16)),
              child: AspectRatio(aspectRatio: 1, child: a.coverMedium.isNotEmpty
                ? Image.network(a.coverBig.isNotEmpty ? a.coverBig : a.coverMedium, fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => Container(color: const Color(0xFF334155), child: const Icon(Icons.album_rounded, color: Colors.white24, size: 48)))
                : Container(color: const Color(0xFF334155), child: const Icon(Icons.album_rounded, color: Colors.white24, size: 48)))),
            Expanded(child: Padding(padding: const EdgeInsets.all(10),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                Text(a.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white, height: 1.2)),
                const SizedBox(height: 4),
                Text(a.artist, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.inter(fontSize: 11, color: Colors.white54)),
                if (a.releaseDate.isNotEmpty) ...[const Spacer(), Row(children: [
                  ...
                  const Spacer(),
                  GestureDetector(onTap: () => openUrl(a.url),
                    child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(Icons.headphones_rounded, color: Color(0xFFA238FF), size: 12), const SizedBox(width: 3),
...
class _C extends StatefulWidget {
  ...
  @override State<_C> createState() => _CS();
}
...
class _FR extends StatefulWidget {
  ...
  @override State<_FR> createState() => _FRS();
}
class _FRS extends State<_FR> {
  ...
  @override void didUpdateWidget(covariant _FR old) { super.didUpdateWidget(old); WidgetsBinding.instance.addPostFrameCallback((_) => _u()); }
    ...
    SizedBox(width: 120, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8),
      ...
      child: TextField(controller: _tc, style: GoogleFonts.inter(fontSize: 11, color: Colors.white),
        decoration: InputDecoration(hintText: widget.hint, hintStyle: GoogleFonts.inter(fontSize: 11, color: Colors.white38),
          ...
          prefixIcon: const Icon(Icons.search_rounded, size: 14, color: Colors.white38), prefixIconConstraints: const BoxConstraints(minWidth: 20)),
...
class _AB extends StatefulWidget {
  ...
  @override State<_AB> createState() => _ABS();
}
...
class _SB extends StatefulWidget {
  ...
  @override State<_SB> createState() => _SBS();
}
class _SBS extends State<_SB> {
  ...
  @override Widget build(BuildContext c) => MouseRegion(cursor: SystemMouseCursors.click,
    onEnter: (_) => setState(() => _h = true), onExit: (_) => setState(() => _h = false),
    child: GestureDetector(onTap: widget.onTap, child: AnimatedContainer(duration: const Duration(milliseconds: 150),
      width: 40, height: 40, decoration: BoxDecoration(color: _h ? const Color(0xFF6366f1) : const Color(0xFF1e293b).withOpacity(0.9),
        shape: BoxShape.circle, border: Border.all(color: const Color(0xFF6366f1).withOpacity(0.4)),
